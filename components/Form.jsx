...
export const Form = () => {
    ...
    const [updateOriginalLink] = useStore((state)=>[state.updateOriginalLink])
    const [updateCustomLink] = useStore((state)=>[state.updateCustomLink])
    ...
    return (
        <form className="flex flex-col w-1/2" onSubmit={handleSubmit(onSubmit)}>
            ...
            <span>{errors?.originalLink?.message}</span>
